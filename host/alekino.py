...
class alekino:
    ...
    def listsMainMenu(self, table):
        for num, val in table.items():
            self.add('alekino', 'main-menu', val, 'None', 'None', 'None', 'None', 'None', True, False)
    ...
    def listsCategoriesMenu(self):
        ...
        if len(match1) > 0:
            ...
            for i in range(len(match1)):
                ...
                self.add('alekino', 'categories-menu', match1[i][1].strip() + ' ' + match1[i][2].strip(), 'None', 'None', catUrl, 'None', 'None', True, False,'1','genres[0]='+match1[i][0])
        xbmcplugin.endOfDirectory(int(sys.argv[1]))
    ...
    def listsItemsTop(self, url):
            ...
            if len(match) > 0:
                for i in range(len(match)):
                        ...
                        #add(self, service, name,               category, title,     iconimage, url, desc, rating, folder = True, isPlayable = True):
                        self.add('alekino', 'playSelectedMovie', 'None', match1[0][2],  match1[0][1], mainUrl+ match1[0][0], 'aaaa', 'None', False, False)
    ...
    def listsItems(self, url, strona='0', filtrowanie=''):
        ...
        if len(match) > 0:
            for i in range(len(match)):
                ...
                if len(match2) > 0:
                    okladka = match2[0]
                self.add('alekino', 'playSelectedMovie', 'None', tytul,  okladka, mainUrl+ match1[0][0], 'aaaa', 'None', False, False)
        log.info('Nastepna strona: '+  urllink)
        self.add('alekino', 'categories-menu', 'Następna', 'None', 'None', url, 'None', 'None', True, False,str(int(strona) + 1), str(filtrowanie))
        xbmcplugin.endOfDirectory(int(sys.argv[1]))
    ...
    def listsItemsPage(self, url):
        ...
        if self.getSizeAllItems(url) > 0  and self.getSizeItemsPerPage(url) > 0:
            ...
            for i in range(int(a)):
                ...
                self.add('alekino', 'items-menu', 'None', title, 'None', destUrl, 'None', 'None', True, False)
        xbmcplugin.endOfDirectory(int(sys.argv[1]))
    ...
    def getSizeAllItems(self, url):
        ...
        openURL.close()
        match = re.compile('<span class="nav_ext">...</span> <a href="http://alekino.tv/filmy/page/(.*?)/">(.*?)</a></div>(.*?)</li>', re.DOTALL).findall(readURL)
        if len(match) == 1:
            numItems = match[0]
        return numItems
    ...
    def handleService(self):
        ...
        if name == None:
            ...
            log.info('Jest Gorące: ')
            self.listsItemsTop('http://alekino.tv/top100?o=mv')
        elif name == 'main-menu' and category == 'Top 100 - ulubione':
            log.info('Jest Gorące: ')
            self.listsItemsTop('http://alekino.tv/top100?o=tf')
